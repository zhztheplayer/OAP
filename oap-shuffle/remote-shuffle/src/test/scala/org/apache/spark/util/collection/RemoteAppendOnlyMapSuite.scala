...
class RemoteAppendOnlyMapSuite extends SparkFunSuite
  ...
  private def createExternalMap[T] = {
    ...
    new RemoteAppendOnlyMap[T, T, ArrayBuffer[T]](
      createCombiner[T], mergeValue[T], mergeCombiners[T], resolver = resolver, context = context)
  }
  ...
  test("RemoteAppendOnlyMap shouldn't fail when forced to spill before calling its iterator") {
    val size = 1000
