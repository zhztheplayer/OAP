...
class IndexDataCacheSeparationSuite extends SharedOapContext with BeforeAndAfterEach{
  ...
  private def fiberCache = fiberCacheManager.getCacheType()
  ...
  test("unit test") {
    ...
    (1 to dataMemorySizeInMB * 2).foreach { i =>
      ...
      val dataFiber =
        TestDataFiberId(
          () => FiberCache.toDataFiberCache(data), s"test data fiber #$fiberGroupId.$i")
    ...
    (1 to indexMemorySizeInMB * 2).foreach { i =>
      ...
      val indexFiber =
        TestIndexFiberId(
          () => FiberCache.toDataFiberCache(data), s"test index fiber #$fiberGroupId.$i")
  ...
  test("remove a fiber is in use") {
    ...
    val dataFiberInUse = TestDataFiberId(
        () => FiberCache.toDataFiberCache(dataInUse), s"test data fiber #${newFiberGroup}.0")
    ...
    val indexFiberInUse = TestIndexFiberId(
      () => FiberCache.toDataFiberCache(dataInUse), s"test index fiber #${newFiberGroup}.0")
  ...
  test("wait for other thread release the fiber") {
    ...
    class IndexFiberTestRunner(i: Int) extends Thread {
      override def run(): Unit = {
        ...
        val fiber = TestIndexFiberId(
          () => FiberCache.toIndexFiberCache(data), s"test index fiber #$fiberGroupId.$i")
        val fiberCache = fiberCacheManager.get(fiber)
        Thread.sleep(2000)
        fiberCache.release()
      }
  ...
  test("add a very large fiber") {
    ...
    val dataException = intercept[AssertionError] {
      ...
      val fiber = TestDataFiberId(
        () => FiberCache.toDataFiberCache(data), s"test fiber #${newFiberGroup}.1")
      val fiberCache = fiberCacheManager.get(fiber)
      fiberCache.release()
    }
    ...
    val indexException = intercept[AssertionError] {
      ...
      val fiber = TestIndexFiberId(
        () => FiberCache.toIndexFiberCache(data), s"test fiber #${newFiberGroup}.1")
      val fiberCache = fiberCacheManager.get(fiber)
      fiberCache.release()
    }
  ...
  test("fiber key equality test") {
    ...
    val dataFiber = TestDataFiberId(
      () => FiberCache.toDataFiberCache(data), s"test data fiber #$fiberGroupId.0")
    ...
    val indexFiber = TestIndexFiberId(
      () => FiberCache.toIndexFiberCache(data), s"test index fiber #$fiberGroupId.0")
  ...
  test("cache guardian remove pending fibers") {
    ...
    val dataFibers = (1 to dataMemorySizeInMB * 2).map { i =>
      val data = generateData(mbSize)
      TestDataFiberId(() => FiberCache.toDataFiberCache(data), s"test fiber #$fiberGroupId.$i")
    }

    val indexMemorySizeInMB = (indexCacheMemorySize / mbSize).toInt
    val indexFibers = (1 to indexMemorySizeInMB * 2).map { i =>
      val data = generateData(mbSize)
      TestIndexFiberId(() => FiberCache.toIndexFiberCache(data), s"test fiber #$fiberGroupId.$i")
    }
  ...
  test("get same fiber simultaneously") {
    ...
    val dataFiber = TestDataFiberId(() => {
      dataLoadTimes += 1
      FiberCache.toDataFiberCache(data)
    }, s"same fiber test")
    def dataWork(): Unit = {
    ...
    val indexFiber = TestIndexFiberId(() => {
      indexLoadTimes += 1
      FiberCache.toIndexFiberCache(data)
    }, s"same fiber test")
    def indexWork(): Unit = {
  ...
  test("get different fiber simultaneously") {
    ...
    val dataRunners = (1 to 6).map { i =>
      ...
      val fiber = TestDataFiberId(() => FiberCache.toDataFiberCache(data), s"different test $i")
      def work(): Boolean = {
        val fiberCache = fiberCacheManager.get(fiber)
    ...
    val indexRunners = (1 to 6).map { i =>
      ...
      val fiber = TestIndexFiberId(() => FiberCache.toIndexFiberCache(data), s"different test $i")
      def work(): Boolean = {
        val fiberCache = fiberCacheManager.get(fiber)
        val flag = fiberCache.toArray sameElements data
        Thread.sleep(100)
  ...
  test("release same fiber simultaneously") {
    ...
    val dataFiber = TestDataFiberId(
      () => FiberCache.toDataFiberCache(data), s"data release test")
    val dataFiberCaches = (1 to 5).map(_ => fiberCacheManager.get(dataFiber))
    assert(dataFiberCaches.head.refCount == 5)

    val indexFiber = TestIndexFiberId(
      () => FiberCache.toIndexFiberCache(data), s"index release test")
  ...
  test("get and release fiber simultaneously") {
    ...
    val data = generateData(kbSize)
    val dataFiber =
      TestDataFiberId(() => FiberCache.toDataFiberCache(data), s"data get release test")
    ...
    val indexFiber =
      TestIndexFiberId(() => FiberCache.toIndexFiberCache(data), s"index get release test")
  ...
  test("get and remove fiber simultaneously") {
    ...
    val dataFiber = TestDataFiberId(
      () => FiberCache.toDataFiberCache(data), s"data fiber get remove test")
    ...
    val indexFiber = TestIndexFiberId(
      () => FiberCache.toIndexFiberCache(data), s"index fiber get remove test")
  ...
  test("test Simple Cache Strategy") {
    ...
    val fiber = TestDataFiberId(
      () => FiberCache.toDataFiberCache(data), "test simple cache fiber")
  ...
  test("LRU blocks memory free") {
    ...
    (1 to dataMemorySizeInMB * 2).foreach { i =>
      val data = generateData(mbSize)
      val fiber = TestDataFiberId(
        () => FiberCache.toDataFiberCache(data), s"test fiber #$fiberGroupId.$i")
      val fiberCache = fiberCacheManager.get(fiber)
      assert(fiberCache.toArray sameElements data)
      fiberCache.release()
    }
    ...
    val indexMemorySizeInMB = (indexCacheMemorySize / mbSize).toInt
    val indexFiberInUse = TestIndexFiberId(
      () => FiberCache.toIndexFiberCache(dataInUse), s"test fiber #${newFiberGroup}.0")
