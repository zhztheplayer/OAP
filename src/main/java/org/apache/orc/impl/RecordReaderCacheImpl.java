...
public class RecordReaderCacheImpl implements RecordReader {
  static final Logger LOG = LoggerFactory.getLogger(RecordReaderCacheImpl.class);
  ...
  // Get the number of rows already Returned
  ...
  public RecordReaderCacheImpl(ReaderImpl fileReader,
  ...
  /**
   * Read the current stripe into memory by OAP's reading mode.
   *
   * @throws IOException
   */
  ...
  public void readBatch(VectorizedRowBatch batch) throws IOException {
    rowInStripe += batch.size;
    reader.setVectorColumnCount(batch.getDataColumnCount());
    reader.nextBatch(batch, (int) rowCountInStripe);
