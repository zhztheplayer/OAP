...
auto n_merge_cache =
    TreeExprBuilder::MakeFunction("appendToCachedBatch", {arg_sum, arg_count}, uint8());
...
auto sch_2 = arrow::schema({f_sum, f_count});
std::vector<std::shared_ptr<Field>> ret_types_2 = {f_sum, f_count};
...
std::vector<std::shared_ptr<gandiva::Expression>> expr_vector_3 = {expr_sum_final,
                                                                   expr_count_final};
auto sch_3 = arrow::schema({f_sum, f_count});
std::vector<std::shared_ptr<Field>> ret_types_3 = {f_sum, f_count};
