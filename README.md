...
* [OAP Overview](#oap-overview)
* [OAP Design](#oap-design)
* [Building](#building)
...
Refer to [OAP Design](./docs/OAP-Design.md).
...
**NOTE**: 1. For spark standalone mode, you have to put `oap-<version>-with-<spark-version>.jar` to both driver and executor since `spark.files` is not working. Also don't forget to update `extraClassPath`.
          ...
          3. The comprehensive guidance and example of OAP configuration can be referred [OAP User guide](./docs/OAP-User-Guide.md). Briefly speaking, the recommended configuration is one executor per one node with fully memory/computation capability.
